...
docker exec -i "${CONTAINER_NAME}" psql "${PSQL_OPTS[@]}" -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" <<EOF
...
docker exec -i "${CONTAINER_NAME}" psql "${PSQL_OPTS[@]}" -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -t <<EOF
SELECT extname, extversion FROM pg_extension WHERE extname = 'vector';
EOF
EXTENSION_COUNT=$(docker exec -i "${CONTAINER_NAME}" psql "${PSQL_OPTS[@]}" -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -tAc "SELECT COUNT(*) FROM pg_extension WHERE extname = 'vector';" | tr -d '[:space:]')
...
PGVECTOR_VERSION=$(docker exec -i "${CONTAINER_NAME}" psql "${PSQL_OPTS[@]}" -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -t -c "SELECT extversion FROM pg_extension WHERE extname = 'vector';" | xargs)
